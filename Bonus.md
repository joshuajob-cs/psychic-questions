...
- Right now data is stored persistently in the database for the case of a server crashing, but authTokens are not stored persistently, so if the server did crash, than players would not be able to login again as the same person. The only legitamite reason to store game data persistently is if someone is playing a game across multiple days (i.e as a bellstarter meet 5 of your classmates each day). Game data only needs to copied to the server once every 1-10 minutes, not every interaction. However it should not be copied over if users are not interacting with the game.
