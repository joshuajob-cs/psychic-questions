...
- Right now game data is stored persistently in the database for the case of a server crashing, but authTokens are not stored persistently, and logging in creates a new game. The only legitamite reason to store game data persistently is if someone is playing a game across multiple days (i.e as a bellstarter meet 5 of your classmates each day). Therefore game data only needs to copied to the server once every minute, and it is better to send all of the data on a game by game basis rather than a player by player basis. However, data should not be copied over if users are not interacting with the game. Authtokens should be stored persistently, so that the user does not need to login every time. If a guest deletes their authToken, they can not log in again. A normal user can get a new auth token with their username and password.
