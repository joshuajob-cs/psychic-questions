...
The user does not recieve the score from the backend, it is always set to 0 at join game. Instead joinGame should pull user.score from the backend when the user logs in again (which would solve deleting the browser data or a server shut down because both would force the user to loes their authToken)
